...
(function () {
  ...
  function findSearchInput() {
    ...
    if (bidButton) {
      ...
      const inputs = Array.from(container.querySelectorAll('input[title*="검색어"], input[placeholder*="검색어"]'))
        .filter(isUsableInput);
      if (inputs.length) return inputs[0];
    }

    return Array.from(document.querySelectorAll('input[title*="검색어"], input[placeholder*="검색어"]'))
      .filter(isUsableInput)[0] || null;
  }
